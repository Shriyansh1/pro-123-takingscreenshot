...
def countFingers(image, hand_landmarks, handNo=0):
	...
	if hand_landmarks:
		...
		angutha_tip_x = int((landmarks[4].x)*width)
		angutha_tip_y = int((landmarks[4].y)*height)
		cv2.line(image,(finger_tip_x,finger_tip_y),(angutha_tip_x,angutha_tip_y),(255,255,0),2)
		
		# Draw a CIRCLE on CENTER of the LINE between FINGER TIP and THUMB TIP
		beech_x = int((finger_tip_x+angutha_tip_x)/2)
		beech_y = int((finger_tip_y+angutha_tip_y)/2)
		cv2.circle(image,(beech_x,beech_y),2,(0,0,255),2)
		
		# Calculate DISTANCE between FINGER TIP and THUMB TIP
		duri = math.sqrt(((finger_tip_x-angutha_tip_x)**2)+((finger_tip_y-angutha_tip_y)**2))
		print(duri)
		...
		relative_mouse_x = (beech_x/width)*screen_width
		relative_mouse_y = (beech_y/height)*screen_height
